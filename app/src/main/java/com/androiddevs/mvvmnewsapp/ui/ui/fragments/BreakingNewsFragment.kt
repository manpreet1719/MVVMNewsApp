...
class BreakingNewsFragment : Fragment(R.layout.fragment_breaking_news) {
            ...
            when(response){
                is Resource.Success -> {
                    ...
                    response.data?.let { newsResponse ->
                        Log.d("breakingNews",response.data.toString())
    ...
    val scrollListener = object : RecyclerView.OnScrollListener(){
        ...
        override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
            ...
            Log.d("scrolling",firstVisibleItemPostion.toString() + " " + visibleItemCount.toString() + " "+ totalItemCount.toString())
            val isAtLastItem =  firstVisibleItemPostion + visibleItemCount >= totalItemCount
            val isNotBegining = firstVisibleItemPostion >= 0
            val isTotalMoreThanVisible = totalItemCount >= QUERY_PAGE_SIZE

            val shouldPaginate = isNotLoadingAndNotLastPage && isAtLastItem && isNotBegining && isTotalMoreThanVisible
                    && isScrolling
            if(shouldPaginate) {
